...
type UAAAuthentication struct {
	connection networkpolicy.Connection
	client     UAAClient
	cache      TokenCache
}
...
func (t *UAAAuthentication) Wrap(innerconnection networkpolicy.Connection) networkpolicy.Connection {
...
func (t *UAAAuthentication) Make(request *http.Request, passedResponse *networkpolicy.Response) error {
	var err error
	...
	if _, ok := err.(networkpolicy.InvalidAuthTokenError); ok {
