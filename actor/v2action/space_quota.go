...
func (actor Actor) GetSpaceQuota(guid string) (SpaceQuota, []string, error) {
	spaceQuota, warnings, err := actor.CloudControllerClient.GetSpaceQuota(guid)

	if _, ok := err.(ccerror.ResourceNotFoundError); ok {
		return SpaceQuota{}, []string(warnings), SpaceQuotaNotFoundError{GUID: guid}
	}

	return SpaceQuota(spaceQuota), []string(warnings), err
}
